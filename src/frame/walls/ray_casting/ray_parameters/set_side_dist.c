...
/*   Updated: 2021/05/20 18:20:44 by mhaddi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_side_dist(t_ray *ray, t_player *player)
{
	if (ray->dir.x < 0)
		ray->side_dist.x = (player->pos.x
				- ray->box.x) * ray->delta_dist.x;
	else
		ray->side_dist.x = (ray->box.x + 1.0
				- player->pos.x) * ray->delta_dist.x;
	if (ray->dir.y < 0)
		ray->side_dist.y = (player->pos.y
				- ray->box.y) * ray->delta_dist.y;
	else
		ray->side_dist.y = (ray->box.y + 1.0
				- player->pos.y) * ray->delta_dist.y;
}
